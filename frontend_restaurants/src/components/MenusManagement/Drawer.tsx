...
import { Drawer, List, ListItem, Button, Typography, Box, IconButton } from '@mui/material';
...
export default function MenuManagementDrawer() {
	...
	return (
		<Drawer
			...
			data-testid="menu-management-drawer"
			sx={{
		...
		>
			<Box className="flex items-center px-4 py-8">
				<IconButton
					component="a"
					href="/home"
					data-testid="menu-management-drawer-back-button"
					sx={{
			...
			<Box className="my-3 flex items-center justify-center">
				...
				<Typography
					...
					fontWeight="bold"
					align="center"
					data-testid="menu-management-drawer-title"
				>
					菜單管理
				</Typography>
			</Box>
			...
			<Box className="mb-[200px] flex h-full flex-col justify-center">
				<List data-testid="menu-management-drawer-list">
					{['概覽', '菜單', '品項控管'].map((text, index) => (
						<ListItem
							key={text}
							disablePadding
							className="my-2 flex-1"
							data-testid="menu-management-drawer-list-item"
						>
							<Box className="w-full" sx={{ padding: theme.spacing(2) }}>
								<Button
									...
									fullWidth
									data-testid="menu-management-drawer-button"
									sx={{
