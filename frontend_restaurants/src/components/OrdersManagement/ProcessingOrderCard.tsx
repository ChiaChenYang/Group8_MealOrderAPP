...
import { delayOrder, finishPreparingOrder } from '@/lib/api/orders/api';
import type { ProcessingOrderType, ProcessingOrderCardProps } from '@/lib/types';
...
function ModalBody({ order, handleClose, setRefreshKey }: ModalBodyProps) {
	...
	const handleSave = async () => {
		if (isFinished) {
			await finishPreparingOrder(order.orderId);
		} else if (delay > 0) {
			await delayOrder(order.orderId, delay);
		}
	...
	return (
		...
		>
			<Box className="flex justify-between">
				...
				>
					{calculateRemainingTime(order.expectedFinishedTime) < 0
						? `超時：${-calculateRemainingTime(order.expectedFinishedTime)} 分鐘`
						: `剩餘：${calculateRemainingTime(order.expectedFinishedTime)} 分鐘`}
				</Typography>
				<CloseIcon
					onClick={handleClose}
					sx={{
...
export default function ProcessingOrderCard({ order, setRefreshKey }: ProcessingOrderCardProps) {
	const theme = useTheme();
