...
import TimePicker from "../component/timepickers";
...
function Checkout() {
  ...
  // };
  const now = new Date();
  const currentDates = now.getDate();
  ...
  const [reservationData, setReservationData] = useState({
    ...
    shop_id: id,
    pick_up_meals: null,
    estimate_time_month: null,
    estimate_time_date: null,
    estimate_time_hour: null,
    estimate_time_minute: null,
  });
  ...
  const handleTimePickerChange = (month, date, hour, minute) => {
    ...
    if (date === currentDates && hour <= currentHours && minute <= currentMinutes) {
  ...
  const handleClick = async (id) => {
    try {
      if (
        reservationData.pick_up_meals === null ||
        reservationData.estimate_time_month === null ||
        reservationData.estimate_time_date === null ||
        reservationData.estimate_time_hour === null ||
        reservationData.estimate_time_minute === null
      ) {
        // Display a warning
        alert("請填寫表單");
