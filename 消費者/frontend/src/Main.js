...
function Main() {
  ...
  return (
    <div className='d-flex justify-content-center vh-100' style={{backgroundColor:"#FFFEFD"}}>
        <div>
        ...
        <div className="mb-3 p-3">
          ...
          <div><img src={mainp} alt="mainp"  style={{width:'90%',position:'absolute',top:"180px",right:"20px"}}/></div>
          <p style={{marginTop:"120px"}}></p>
          <CustomizedInputBase />
        </div>
        <div style={{marginLeft:"10px",marginBottom:"10px",width:"95%"}}>
          <NestedGrid />
        </div>
        <div className="mb-3 p-3">
        {Object.keys(restaurant_information).map((key) => (
          ...
          <div key={key} className="mb-3" style={{display: 'flex', alignItems: 'center',borderRadius:"15px",border:"2px solid #F0F0F0"}}>
            <div style={{ flex: '0 0 60%'}}>
              <img src={imagei} alt={restaurant_information[key].name} style={{ height:"100%",width: '100%',borderRadius:"15px 0px 0px 15px" }} />
            </div>
            <div style={{ flex: '0 0 40%' ,padding: '0.9rem',borderRadius:"0px 15px 15px px"}}>
              <h4><strong>{restaurant_information[key].name}</strong></h4>
