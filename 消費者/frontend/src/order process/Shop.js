    ...
    <div>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
          ...
          {Object.keys(item.items).map(itemId => {
            ...
            return (
              <div key={itemId} style={{ paddingLeft: '20px'}}>
                <scan style={{ fontSize: '25px' }}><strong>{currentItem.name}</strong></scan>
                <scan style={{ marginLeft: '150px' }}> <img src={currentItem.image} style={{ width: '30%' }} alt='圖片'/> </scan>
                <p><Counter initialValue={currentItem.quantity} onCountChange={(newCount) => handleCountChange(itemId, newCount)} /></p>
                <p style={{ marginLeft: '300px' }}>$ {currentItem.price * counts[itemId]}</p>
        ...
        <p style={{ marginLeft: '20px' }}>
          小計<scan style={{ marginLeft: '280px' }}>{total}</scan>
        </p>
      </strong>
          ...
          <p></p>
          <button className='rounded' onClick={() => handleClick(item.shop_id)} style={{ border:'none',height:"30px",width: '90%', marginLeft: '20px', backgroundColor: "#F4B63D" }}>
            <strong>繼續點餐</strong>
          </button>
          <p></p>
        <button className='rounded' onClick={handleClick2} style={{ border:'none',height:"30px",width: '90%', marginLeft: '20px', backgroundColor: "#F4B63D" }}>
          <strong>前往結帳</strong>
        </button>
